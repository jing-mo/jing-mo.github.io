...
function startInit() {
  ...
  setTimeout(startInit(),1600);var mobile_sidebar_menus = document.getElementById("mobile-sidebar-menus");
...
function bot_ui_ini(){
  ...
  botui.message.add({delay:800,content:"Hi, there"})
    .then(function(){
      ...
      sure=function(){
      botui.message.add({delay:600,content:"我来啦"})
      .then(function() {
        botui.action.button({delay:1000,action:[{text: "你的爱好有什么",value: "second"},{text: "哦，那加油哦",value:"skip"}]})
        .then(function(a){
          "second"==a.value&&secondpart()
          "skip"==a.value&&thanks()
        })
      })
    },
    ...
    thanks = function() {
      botui.message.add({delay:600,content:"谢谢"});
    },

    end=function(){
      botui.message.add({delay:600,content:"哦，那好吧，如果喜欢小站就动手做一个"})
    },

    secondpart=function(){
      botui.message.add({delay:1500,content:"爱好呀。。。"})
        .then(function(){
          botui.message.add({delay:1500,content:"拼模型"})
            .then(function(){
              botui.message.add({delay:1200,content:"打代码"})
                .then(function(){
                  botui.message.add({delay:1500,content:"看书"})
                    .then(function(){
                      botui.message.add({delay:1500,content:"大概就这些，嘿嘿嘿0w0"})
                        .then(function(){botui.message.add({delay:1800,content:"还有什么想问的吗？"})
                          .then(function(){
                            botui.action.button({delay:1100,action:[{text:"如果现在开始学前端的话应该从什么开始",value:"howstudy"},{text:"没有了",value:"skip"}]})
                            .then(function(a){
                                "howstudy"==a.value&&thirdpart()
                                "skip"==a.value&&last()
                            })
                          })
                        })
    ...
    last = function() {
      botui.message.add({delay:600,content:"好的"})
    };
 seeyou = function() {
      botui.message.add({delay:1500,content:"希望你玩得高兴！"})
    }         
}
//rebuild by neat